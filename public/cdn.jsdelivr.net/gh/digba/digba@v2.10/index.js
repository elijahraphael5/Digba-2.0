  ...
  const openNav = () => {
    ...
    tl.clear()
      ...
      .fromTo(
        ...
        { scaleX: 1, stagger: 0.1, duration: 1 },
        '<'
      )
      .fromTo(
        menuIndexs,
...
function Signature() {
  if (ranHomeLoader === false) {
    console.log(
      '%cCredits: Digba – https://www.digbaogunbiyi.com',
...
function skipLoaderIfBackForward(loaderWrap) {
...
function initHomeLoader() {
  ...
  if (skipLoaderIfBackForward(loaderWrap)) return;
  let loaderVisualWrapper = loaderWrap.querySelectorAll(
    '.loader_visual_wrapper'
  );
...
function initHome(next) {
  ...
  tl.from(title, { yPercent: 110, duration: 0.6, ease: 'power2.out' }, '< 0.3');
}
...
////////// NEXT.JS BOOT (replaces barba transitions) //////////
...
(function () {
  ...
  function runInit() {
    ...
    safe(resetThemeLight);

    if (ranHomeLoader !== true) {
      var loaderInits = {
        home: initHomeLoader,
        about: initAboutLoader,
        works: initWorksLoader,
        contact: initContactLoader,
        projects: initProjectLoader,
        '404': initErrorLoader,
      };
      var loaderInit = loaderInits[namespace] || initHomeLoader;
      safe(function () { loaderInit(); });
      safe(initMenu);
      safe(mouseMoveCursor);

      if (namespace === 'home') {
        safe(initThemeAnimation);
        safe(function () { initSectionFade(document); });
      }
      if (namespace === 'projects') {
        safe(function () { initStopmotion(document); });
        safe(function () { initFooterWorks(document); });
        safe(function () { initVisualSlidein(document); });
      }
    }

    safe(function () { updateYear(document); });
  ...
  function boot() {
    var start = function () { setTimeout(runInit, 300); };
    ...
    window.addEventListener('load', function () {
      setTimeout(function () { safe(function () { ScrollTrigger.refresh(); }); }, 500);
    });
  }
  boot();
})();
